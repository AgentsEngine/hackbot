...
`packages/hackbot` will use it internally, but the package itself should be generic: Drizzle + Effect + SQLite. No opencode paths, migrations, tables, transaction hooks, post-commit behavior, or domain language should live in this package.
...
- Opencode-specific path/channel selection stays in `packages/hackbot`.

## Opencode Adoption Notes

These are not package requirements, but they matter for the later opencode adoption PR.

The current `packages/hackbot/src/storage/db.ts` has two non-obvious semantics that the opencode wrapper must preserve when it consumes this adapter:
...
1. Add `hackbot-effect-drizzle-sqlite` with a minimal in-memory/file SQLite test schema.
...
4. Add `hackbot-effect-drizzle-sqlite` as a dependency of `packages/hackbot`.
5. Port `packages/hackbot/src/storage/db.ts` to be a thin compatibility wrapper over the adapter plus opencode-specific transaction/post-commit context.
...
- Do CLI/admin raw SQL and sqlite shell stay in `packages/hackbot`, or does the storage package expose backend capabilities for them?
...
- Do not migrate `packages/hackbot` yet except possibly adding the dependency if needed for typechecking.
