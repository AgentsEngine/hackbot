...
`packages/hackbot/src/cli/cmd/tui` into a self-contained workspace package while
...
generic presentation libraries. It must not depend on `packages/hackbot`,
`packages/cli`, or `hackbot-core`.
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/hackbot`.
- Do not replace private `packages/hackbot` imports with `hackbot-core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `hackbot-tui`.
...
```text
packages/hackbot/src/cli/cmd/tui
```

Its private dependency on `packages/hackbot` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/hackbot/src/*`.
...
Status: Completed. The private `hackbot-tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `hackbot-tui`.
...
- The package has no dependency on `opencode`, `hackbot-cli`, or
  `hackbot-core`.
...
primitives now live in `hackbot-tui`. The legacy host consumes explicit
...
and build inputs through `hackbot-tui/runtime`. Movable app, component,
...
`hackbot-tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/hackbot` initially.
...
canonical in `hackbot-tui`. Configured references resolve through the new
...
`hackbot-tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/hackbot`.
...
- The package has no imports from `packages/hackbot`, `packages/cli`, or
  `hackbot-core`.
...
public `hackbot-tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/hackbot`.
- Keep the legacy embedded worker and server startup in `packages/hackbot`.
- Change those adapters to load config, create transport inputs, and call the
  public `hackbot-tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/hackbot`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `hackbot-tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `hackbot-core`, or either
  executable package.
...
cd packages/hackbot && bun typecheck
...
rg 'hackbot-core|packages/hackbot|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/hackbot/src packages/hackbot/test
```
...
- Build the current-platform `packages/hackbot` binary.
