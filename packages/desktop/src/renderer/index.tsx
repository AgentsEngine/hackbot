    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.HACKBOT_CHANNEL === "prod" &&
...
const emitDeepLinks = (urls: string[]) => {
  ...
  window.__HACKBOT__ ??= {}
  const pending = window.__HACKBOT__.deepLinks ?? []
  window.__HACKBOT__.deepLinks = [...pending, ...urls]
...
function DesktopRoot(props: { windowState: DesktopWindowState }) {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("hackbot.global.dat").getItem("language")
