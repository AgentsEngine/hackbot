...
export async function spawnWslSidecar(
  ...
  if (!opencode) throw new Error(`hackbot is not installed in ${distro}`)
  ...
  const script = [
    ...
    "export PATH",
    "export WSLENV=",
    "export HACKBOT_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export HACKBOT_CLIENT=desktop",
    `export HACKBOT_SERVER_USERNAME=${shellEscape(username)}`,
    `export HACKBOT_SERVER_PASSWORD=${shellEscape(password)}`,
