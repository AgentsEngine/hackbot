...
const TAURI_APP_IDS: Record<string, string> = {
  dev: "ai.hackbot.desktop.dev",
  beta: "ai.hackbot.desktop.beta",
  prod: "ai.hackbot.desktop",
}
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.hackbot.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `hackbot.settings.dat` is special: it maps to the `hackbot.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"hackbot.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // hackbot.settings.dat → the electron settings store ("hackbot.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "hackbot.global.dat").
  const storeName = filename === "hackbot.settings.dat" ? "hackbot.settings" : filename
