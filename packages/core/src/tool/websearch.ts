...
export class ConfigService extends Context.Service<ConfigService, Config>()("@hackbot/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.HACKBOT_WEBSEARCH_PROVIDER === "exa" || process.env.HACKBOT_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.HACKBOT_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("HACKBOT_EXPERIMENTAL") || truthy("HACKBOT_ENABLE_EXA") || truthy("HACKBOT_EXPERIMENTAL_EXA"),
    enableParallel: truthy("HACKBOT_ENABLE_PARALLEL") || truthy("HACKBOT_EXPERIMENTAL_PARALLEL"),
              ...
              const text =
                provider === "exa"
                  ...
                  : yield* callMcp(
                      ...
                      {
                        "User-Agent": `hackbot/${InstallationVersion}`,
