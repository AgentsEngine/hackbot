...
import { ModelsDevPlugin } from "@hackbot/core/plugin/models-dev"
...
describe("ModelsDevPlugin", () => {
  ...
  it.effect("registers key methods for providers with environment variables", () =>
    Effect.acquireUseRelease(
      Effect.sync(() => {
        const previous = {
          path: Flag.HACKBOT_MODELS_PATH,
          disabled: Flag.HACKBOT_DISABLE_MODELS_FETCH,
        }
        Flag.HACKBOT_MODELS_PATH = path.join(import.meta.dir, "fixtures", "models-dev.json")
      ...
      (previous) =>
        Effect.sync(() => {
          Flag.HACKBOT_MODELS_PATH = previous.path
          Flag.HACKBOT_DISABLE_MODELS_FETCH = previous.disabled
        }),
    ),
  )
})
