...
export const HACKBOT_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const HACKBOT_MODE_KEY = "hackbot.mode"
...
export function createOpencodeModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(HACKBOT_MODE_KEY, HACKBOT_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(HACKBOT_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(HACKBOT_MODE_KEY, stack.at(-1)?.mode ?? HACKBOT_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? HACKBOT_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(HACKBOT_MODE_KEY, undefined)
