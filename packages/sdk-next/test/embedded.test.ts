...
import type { hackbotEvent } from "../src"

test("embedded client uses the real router and handlers", async () => {
  const directory = await mkdtemp(join(tmpdir(), "opencode-embedded-"))
  const database = Flag.HACKBOT_DB
  Flag.HACKBOT_DB = join(directory, "opencode.sqlite")
  const { AbsolutePath, Agent, Location, Model, hackbot, Prompt, Provider, Session, Tool } = await import("../src")
  const sessionID = Session.ID.make(`ses_embedded_${crypto.randomUUID()}`)
  const model = Model.Ref.make({ id: Model.ID.make("embedded"), providerID: Provider.ID.make("test") })

  try {
    const program = Effect.gen(function* () {
      const opencode = yield* hackbot.create()
...
test("Location-owned runner events reach the ready global client", async () => {
  ...
  const database = Flag.HACKBOT_DB
  Flag.HACKBOT_DB = join(directory, "opencode.sqlite")
  const { AbsolutePath, Location, hackbot, Prompt, Session } = await import("../src")
  const sessionID = Session.ID.make(`ses_embedded_${crypto.randomUUID()}`)
  ...
  try {
    const program = Effect.gen(function* () {
      ...
      const prompted = yield* Deferred.make<hackbotEvent>()
...
test("independent embedded hosts do not share live notifications", async () => {
  ...
  const database = Flag.HACKBOT_DB
  Flag.HACKBOT_DB = join(directory, "opencode.sqlite")
  const { AbsolutePath, Agent, Location, hackbot, Session } = await import("../src")
  const sessionID = Session.ID.make(`ses_embedded_${crypto.randomUUID()}`)

  try {
    const program = Effect.gen(function* () {
      const first = yield* hackbot.create()
      const second = yield* hackbot.create()
      ...
      const observe = (ready: Latch.Latch, event: Latch.Latch) =>
        Stream.runForEach((notification: hackbotEvent) =>
...
test("embedded client is available as a Layer service", async () => {
  ...
  const database = Flag.HACKBOT_DB
  Flag.HACKBOT_DB = join(directory, "opencode.sqlite")
  const { AbsolutePath, Location, hackbot, Session } = await import("../src")
  const sessionID = Session.ID.make(`ses_embedded_${crypto.randomUUID()}`)

  try {
    const created = await Effect.runPromise(
      Effect.gen(function* () {
        const opencode = yield* hackbot.Service
        return yield* opencode.sessions.create({
          id: sessionID,
          location: Location.Ref.make({ directory: AbsolutePath.make(directory) }),
        })
      }).pipe(Effect.provide(hackbot.layer), Effect.scoped),
    )
