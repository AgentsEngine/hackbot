...
For at fejlfinde problemer med Hackbot, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\hackbot\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `hackbot --log-level DEBUG`.

---

## Lagring

hackbot gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/hackbot/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\hackbot`
...
Hackbot Desktop kører en lokal Hackbot-server (`opencode-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `Hackbot`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/hackbot/hackbot.jsonc` (eller `~/.config/hackbot/hackbot.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/hackbot/hackbot.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\hackbot\hackbot.jsonc`
...
Hackbot kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/hackbot/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\hackbot\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så Hackbot kan genopbygge den.

1. Afslut Hackbot Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/hackbot`
- **Linux**: slet `~/.cache/hackbot` (eller kør `rm -rf ~/.cache/hackbot`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\hackbot`

3. Start Hackbot Desktop på ny.

---

### Løs problemer med serverforbindelse

Hackbot Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `hackbot.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `HACKBOT_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `HACKBOT_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver Hackbot Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i Hackbot.

---

### Meddelelser vises ikke

Hackbot Desktop viser kun systemvarsler når:

- varsler er aktiveret for Hackbot i dine OS-indstillinger, og
...
1. Afslut Hackbot Desktop.
2. Find og slet disse filer (de findes i Hackbot Desktop-appens datamappe):

- `hackbot.settings.dat` (skrivebordsstandardserver URL)
- `hackbot.global.dat` og `hackbot.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med Hackbot:
...
For at finde ud af hvilke modeller du har adgang til, kør `hackbot models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\hackbot`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. hackbot installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\hackbot`

2. Start hackbot på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge hackbot til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
hackbot vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
