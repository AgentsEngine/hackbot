...
Du kan tilføje eksterne værktøjer til Hackbot ved hjælp af _Model Context Protocol_ eller MCP. Hackbot understøtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i din [Hackbot Config](https://hackbot.ai/docs/config/) under `mcp`. Tilføj hver MCP med et unikt navn. Du kan henvise til denne MCP ved navn, når du spørger LLM.
...
Organisationer kan levere standard MCP-servere via deres `.well-known/hackbot`-slutpunkt. Disse servere kan være deaktiveret som standard, hvilket giver brugerne mulighed for at tilmelde sig dem, de har brug for.
...
Hackbot håndterer automatisk OAuth-godkendelse for eksterne MCP-servere. Når en server kræver godkendelse, vil Hackbot:
...
Hvis serveren kræver godkendelse, vil Hackbot bede dig om at godkende, når du prøver at bruge den første gang. Hvis ikke, kan du [manually trigger the flow](#authenticating) med `hackbot mcp auth <server-name>`.
...
Kommandoen `mcp auth` åbner din browser for godkendelse. Når du har godkendt, vil Hackbot opbevare tokens sikkert i `~/.local/share/hackbot/mcp-auth.json`.
...
Dine MCPs er tilgængelige som værktøjer i **TK\_** sammen med indbyggede værktøjer. Så du kan administrere dem gennem Hackbot-konfigurationen som ethvert andet værktøj.
...
Dette åbner et browservindue for at fuldføre OAuth-flowet og forbinde Hackbot til din Sentry-konto.
