---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i Hackbot.
---

import config from "../../../../config.mjs"
export const console = config.console

Hackbot bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i Hackbot-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/hackbot/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i Hackbot-konfigurationen.
...
Hackbot Zen er en liste over modeller leveret af Hackbot-teamet, som er blevet testet og verificeret til at fungere godt med Hackbot. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med Hackbot Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg hackbot og gå til [hackbot.ai/auth](https://hackbot.ai/auth).
...
Det fungerer som alle andre udbydere i Hackbot og er helt valgfrit at bruge.

---

## Hackbot Go

Hackbot Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af Hackbot-teamet, som er testet og verificeret til at fungere godt med Hackbot.

1. Kør kommandoen `/connect` i TUI, vælg `Hackbot Go`, og gå til [hackbot.ai/auth](https://hackbot.ai/zen).
...
Sådan bruger du Amazon Bedrock med Hackbot:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører hackbot:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `hackbot.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i Hackbot understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere hackbot til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at hackbot skal fungere korrekt.
   :::
...
gemme tokenet i hackbot auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
Hackbot bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse Hackbot
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`hackbot.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Sådan bruger du GitHub Copilot-abonnementet med Hackbot:
...
Sådan bruger du Google Vertex AI med Hackbot:
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem hackbot, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin for automatisk at logge hver Hackbot-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver Hackbot-samtale opført som en separat session.
...
Du kan konfigurere hackbot til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere hackbot til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere hackbot til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for Hackbot. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/hackbot) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med Hackbot:
...
3. Kopier API-nøglen til brug i Hackbot.
...
6. **Vigtigt**: Før du bruger skymodeller i Hackbot, skal du hente modelinformationen lokalt:
...
Hackbot Zen er en liste over testede og verificerede modeller leveret af Hackbot-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>Hackbot Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **Hackbot Zen**.

   ```txt
   /connect
   ```

3. Indtast Hackbot API-nøglen.
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Hackbot:
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med hackbot. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `hackbot.json`-filen i projektkataloget:
...
`limit`-felterne lader Hackbot forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `hackbot auth list` for at se om legitimationsoplysningerne
