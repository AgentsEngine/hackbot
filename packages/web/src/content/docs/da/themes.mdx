...
Med Hackbot kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger Hackbot vårt eget `hackbot`-tema.
...
Hackbot støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/hackbot/themes/*.json` eller `$XDG_CONFIG_HOME/hackbot/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.hackbot/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.hackbot/themes/*.json`
