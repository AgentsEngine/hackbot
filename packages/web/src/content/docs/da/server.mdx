...
Kommandoen `hackbot serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en opencode-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `hackbot.local` |
...
Indstil `HACKBOT_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `hackbot`, eller indstil `HACKBOT_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `hackbot serve` og `hackbot web`.
...
Når du kører `hackbot` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug opencode-serveren til at interagere med hackbot programmatisk.
:::

Denne arkitektur lader hackbot understøtte flere klienter og lader dig interagere med hackbot programmatisk.

Du kan køre `hackbot serve` for at starte en selvstændig server. Hvis du har
hackbot TUI kørende, vil `hackbot serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af Hackbot [IDE](/docs/ide) plugins.
...
Hackbot-serveren viser følgende API'er.
