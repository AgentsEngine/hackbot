---
title: Plugins
description: Skriv dine egne plugins for at udvide Hackbot.
---

Plugins giver dig mulighed for at udvide Hackbot ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre OpenCodes standardadfærd.
...
- `.hackbot/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/hackbot/node_modules/`.
...
1. Global konfiguration (`~/.config/hackbot/hackbot.json`)
2. Projektkonfiguration (`hackbot.json`)
3. Global plugin-mappe (`~/.config/hackbot/plugins/`)
4. Projekt plugin bibliotek (`.hackbot/plugins/`)
...
Hackbot kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En hackbot SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide hackbot.
...
:::note
Hvis du bruger Hackbot desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå hackbot i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til hackbot:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som hackbot kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for hackbot sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://hackbot.ai/docs/sdk) for detaljer.
