...
You can add external tools to Hackbot using the _Model Context Protocol_, or MCP. Hackbot supports both local and remote servers.
...
You can define MCP servers in your [Hackbot Config](https://hackbot.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/hackbot` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
Hackbot automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Hackbot will:
...
If the server requires authentication, Hackbot will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `hackbot mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, Hackbot will store the tokens securely in `~/.local/share/hackbot/mcp-auth.json`.
...
Your MCPs are available as tools in Hackbot, alongside built-in tools. So you can manage them through the Hackbot config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect Hackbot to your Sentry account.
