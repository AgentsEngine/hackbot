...
As ferramentas permitem que o LLM execute ações em sua base de código. O hackbot vem com um conjunto de ferramentas integradas, mas você pode estendê-lo com [ferramentas personalizadas](/docs/custom-tools) ou [servidores MCP](/docs/mcp-servers).
...
Aqui estão todas as ferramentas integradas disponíveis no hackbot.
...
:::note
Esta ferramenta está disponível apenas quando `HACKBOT_EXPERIMENTAL_LSP_TOOL=true` (ou `HACKBOT_EXPERIMENTAL=true`).
:::
...
:::note
Esta ferramenta está disponível apenas ao usar o provedor hackbot ou quando a variável de ambiente `HACKBOT_ENABLE_EXA` está definida como qualquer valor verdadeiro (por exemplo, `true` ou `1`).

Para habilitar ao iniciar o hackbot:
