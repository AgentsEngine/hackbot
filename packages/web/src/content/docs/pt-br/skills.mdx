...
As habilidades do agente permitem que o hackbot descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O hackbot pesquisa nesses locais:

- Configuração do projeto: `.hackbot/skills/<name>/SKILL.md`
- Configuração global: `~/.config/hackbot/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o hackbot sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.hackbot/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/hackbot/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.hackbot/skills/git-release/SKILL.md` assim:
...
O hackbot lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `hackbot.json`:
...
**Para agentes embutidos** (em `hackbot.json`):
