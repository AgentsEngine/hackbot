---
title: Servidor
description: Interaja com o servidor hackbot via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `hackbot serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente hackbot pode usar.
...
```bash
hackbot serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `hackbot.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
hackbot serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `HACKBOT_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `hackbot`, ou defina `HACKBOT_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `hackbot serve` quanto ao `hackbot web`.

```bash
HACKBOT_SERVER_PASSWORD=your-password hackbot serve
```
...
Quando você executa `hackbot`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor hackbot para interagir com o hackbot programaticamente.
:::

Essa arquitetura permite que o hackbot suporte múltiplos clientes e permite que você interaja com o hackbot programaticamente.

Você pode executar `hackbot serve` para iniciar um servidor autônomo. Se você tiver o TUI do hackbot em execução, `hackbot serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do hackbot [IDE](/docs/ide).
...
O servidor hackbot expõe as seguintes APIs.
