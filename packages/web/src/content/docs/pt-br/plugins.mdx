---
title: Plugins
description: Escreva seus próprios plugins para estender o hackbot.
---

Plugins permitem que você estenda o hackbot conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do hackbot.
...
- `.hackbot/plugins/` - Plugins em nível de projeto
- `~/.config/hackbot/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/hackbot/node_modules/`.
...
1. Configuração global (`~/.config/hackbot/hackbot.json`)
2. Configuração do projeto (`hackbot.json`)
3. Diretório de plugins global (`~/.config/hackbot/plugins/`)
4. Diretório de plugins do projeto (`.hackbot/plugins/`)
...
O hackbot executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do hackbot para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o hackbot.
...
:::note
Se você estiver usando o aplicativo desktop hackbot, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o hackbot de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao hackbot:
...
O helper `tool` cria uma ferramenta personalizada que o hackbot pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o hackbot junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://hackbot.ai/docs/sdk) para detalhes.
