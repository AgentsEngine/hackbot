...
Com o hackbot, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o hackbot usa nosso próprio tema `hackbot`.
...
O hackbot vem com vários temas embutidos.
...
- Querem que o hackbot corresponda à aparência do seu terminal
...
O hackbot suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/hackbot/themes/*.json` ou `$XDG_CONFIG_HOME/hackbot/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.hackbot/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.hackbot/themes/*.json`
