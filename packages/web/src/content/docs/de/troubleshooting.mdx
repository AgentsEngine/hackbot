...
Wenn Hackbot Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\hackbot\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `hackbot --log-level DEBUG`.

---

## Speicher

hackbot speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/hackbot/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\hackbot`
...
Hackbot Desktop startet im Hintergrund einen lokalen Hackbot-Server (`opencode-cli`-Sidecar).
...
- Nur macOS: `Hackbot`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/hackbot/hackbot.jsonc` (oder `~/.config/hackbot/hackbot.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/hackbot/hackbot.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\hackbot\hackbot.jsonc`
...
Hackbot kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/hackbot/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\hackbot\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit Hackbot ihn neu aufbauen kann.

1. Beende Hackbot Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/hackbot`
- **Linux**: loesche `~/.cache/hackbot` (oder fuehre aus: `rm -rf ~/.cache/hackbot`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\hackbot`

3. Starte Hackbot Desktop neu.

---

### Server-Verbindungsprobleme beheben

Hackbot Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `hackbot.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `HACKBOT_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `HACKBOT_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt Hackbot Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit Hackbot-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

Hackbot Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer Hackbot in den OS-Einstellungen aktiviert sind, und
...
1. Beende Hackbot Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von Hackbot Desktop):

- `hackbot.settings.dat` (desktop default server URL)
- `hackbot.global.dat` und `hackbot.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit Hackbot hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `hackbot models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\hackbot`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. hackbot installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\hackbot`

2. Starte hackbot neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt hackbot, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
hackbot erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
