...
Hackbot Go ist ein kostengünstiges Abonnement — **5 $ für deinen ersten Monat**, danach **10 $/Monat** —, das dir zuverlässigen Zugriff auf beliebte offene Coding-Modelle bietet.

Go funktioniert wie jeder andere Provider in Hackbot. Du abonnierst Hackbot Go und
erhältst deinen API-Key. Es ist **völlig optional** und du musst es nicht nutzen, um
Hackbot zu verwenden.
...
Hackbot Go bietet dir Zugriff auf diese Modelle für **5 $ im ersten Monat**, danach **10 $/Monat**.

---

## Wie es funktioniert

Hackbot Go funktioniert wie jeder andere Provider in Hackbot.

1. Du meldest dich bei **<a href={console}>Hackbot Zen</a>** an, abonnierst Go und kopierst deinen API-Key.
2. Du führst den Befehl `/connect` in der TUI aus, wählst `Hackbot Go` und fügst deinen API-Key ein.
3. Führe `/models` in der TUI aus, um die Liste der über Go verfügbaren Modelle zu sehen.

:::note
Nur ein Mitglied pro Workspace kann Hackbot Go abonnieren.
:::
...
Hackbot Go beinhaltet die folgenden Limits:
...
Die [Modell-ID](/docs/config/#models) in deiner Hackbot Config verwendet das Format `opencode-go/<model-id>`. Für Kimi K3 würdest du beispielsweise `opencode-go/kimi-k3` in deiner Config verwenden.
...
Wir haben Hackbot Go entwickelt, um:
...
4. **Keinen Lock-in-Effekt** zu haben, indem wir dir ermöglichen, auch jeden anderen Provider mit Hackbot zu nutzen.
