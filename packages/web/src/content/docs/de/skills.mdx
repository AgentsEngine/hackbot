...
Agent Skills erlauben Hackbot, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
Hackbot sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft Hackbot vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.hackbot/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/hackbot/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.hackbot/skills/git-release/SKILL.md` zum Beispiel so:
...
Hackbot listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `hackbot.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `hackbot.json`):
