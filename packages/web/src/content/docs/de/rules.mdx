---
title: Regeln
description: Lege eigene Anweisungen fuer hackbot fest.
---

Du kannst hackbot ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in hackbot den Befehl `/init` aus.
...
So kann hackbot sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht hackbot sie zu erweitern.
...
hackbot kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/hackbot/AGENTS.md` speichern.
...
Wenn du von Claude Code kommst, unterstuetzt Hackbot dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/hackbot/AGENTS.md` existiert)
...
Beim Start sucht hackbot in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/hackbot/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `hackbot.json` oder in `~/.config/hackbot/hackbot.json` angeben.
...
hackbot parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit hackbot.json

Empfohlen ist das `instructions`-Feld in der `hackbot.json`:
...
Du kannst hackbot explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `hackbot.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
