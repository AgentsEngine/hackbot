...
Mit Hackbot kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet Hackbot das `hackbot`-Theme.
...
Hackbot liefert mehrere eingebaute Themes mit.
...
- Hackbot optisch ans Terminal anpassen wollen
...
Hackbot unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/hackbot/themes/*.json` oder `$XDG_CONFIG_HOME/hackbot/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.hackbot/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.hackbot/themes/*.json`
