---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um Hackbot zu erweitern.
---

Mit Plugins können Sie Hackbot erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von Hackbot zu ändern.
...
- `.hackbot/plugins/` – Plugins auf Projektebene
- `~/.config/hackbot/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/hackbot/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/hackbot/hackbot.json`)
2. Projektkonfiguration (`hackbot.json`)
3. Globales Plugin-Verzeichnis (`~/.config/hackbot/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.hackbot/plugins/`)
...
Hackbot führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein Hackbot-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie Hackbot erweitern können.
...
:::note
Wenn Sie die Hackbot-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass Hackbot `.env`-Dateien liest:
...
Plugins können Hackbot auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das Hackbot aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für Hackbot zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://hackbot.ai/docs/sdk).
