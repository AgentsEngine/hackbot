...
Avec Hackbot, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, Hackbot utilise notre propre thème `hackbot`.
...
Hackbot est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que Hackbot corresponde à l'apparence de leur terminal
...
Hackbot prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/hackbot/themes/*.json` ou `$XDG_CONFIG_HOME/hackbot/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.hackbot/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.hackbot/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/hackbot/themes
vim ~/.config/hackbot/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .hackbot/themes
vim .hackbot/themes/my-theme.json
```
...
{
  "$schema": "https://hackbot.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
