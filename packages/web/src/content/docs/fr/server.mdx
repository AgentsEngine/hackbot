---
title: Serveur
description: Interagissez avec le serveur hackbot via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `hackbot serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client hackbot peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `hackbot.local` |
...
Définissez `HACKBOT_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `hackbot`, ou définissez `HACKBOT_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `hackbot serve` et à `hackbot web`.
...
Lorsque vous exécutez `hackbot`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur hackbot pour interagir avec hackbot par programmation.
:::

Cette architecture permet à hackbot de prendre en charge plusieurs clients et vous permet d'interagir avec hackbot par programme.

Vous pouvez exécuter `hackbot serve` pour démarrer un serveur autonome. Si vous avez le
hackbot TUI en cours d'exécution, `hackbot serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins Hackbot [IDE](/docs/ide).
...
Le serveur hackbot expose les API suivantes.
