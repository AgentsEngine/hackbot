---
title: Plugins
description: Écrivez vos propres plugins pour étendre Hackbot.
---

Les plugins vous permettent d'étendre Hackbot en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de Hackbot.
...
- `.hackbot/plugins/` - Plugins au niveau du projet
- `~/.config/hackbot/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/hackbot/node_modules/`.
...
1. Configuration globale (`~/.config/hackbot/hackbot.json`)
2. Configuration du projet (`hackbot.json`)
3. Répertoire global des plugins (`~/.config/hackbot/plugins/`)
4. Répertoire des plugins du projet (`.hackbot/plugins/`)
...
Hackbot exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK hackbot pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre hackbot.
...
:::note
Si vous utilisez l'application de bureau Hackbot, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez hackbot de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à hackbot :
...
L'assistant `tool` crée un outil personnalisé que hackbot peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour hackbot aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://hackbot.ai/docs/sdk) pour plus de détails.
