...
Možete dodati vanjske alate u Hackbot koristeći _Model Context Protocol_, ili MCP. Hackbot podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [Hackbot Config](https://hackbot.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/hackbot`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
Hackbot automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, Hackbot će:
...
Ako server zahtijeva autentifikaciju, Hackbot će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `hackbot mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, Hackbot sigurno čuva tokene u `~/.local/share/hackbot/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u Hackbot, zajedno s ugrađenim alatima. Možete njima upravljati kroz Hackbot konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete Hackbot sa vašim Sentry nalogom.
