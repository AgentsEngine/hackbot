...
U Hackbot mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, Hackbot koristi nasu `hackbot` temu.
...
Hackbot dolazi sa vise ugradenih tema.
...
- Zele da Hackbot odgovara izgledu njihovog terminala
...
Hackbot podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/hackbot/themes/*.json` ili `$XDG_CONFIG_HOME/hackbot/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.hackbot/themes/*.json`
4. **Trenutni radni direktorij** - `./.hackbot/themes/*.json`
