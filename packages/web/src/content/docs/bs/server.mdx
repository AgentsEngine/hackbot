---
title: Server
description: Komunicirajte s hackbot serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `hackbot serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji hackbot klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `hackbot.local` |
...
Postavite `HACKBOT_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `hackbot`, ili postavite `HACKBOT_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `hackbot serve` i za `hackbot web`.
...
Kada pokrenete `hackbot`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite hackbot server za programsku interakciju sa opencode-om.
:::

Ova arhitektura omogucava hackbot podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `hackbot serve` da startate standalone server. Ako je hackbot TUI vec pokrenut, `hackbot serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste Hackbot [IDE](/docs/ide) pluginovi.
...
hackbot server izlaže sljedece API-je.
