---
title: Dodaci
description: Napišite vlastite dodatke za proširenje Hackbot.
---

Dodaci vam omogućavaju da proširite Hackbot spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja Hackbot.
...
- `.hackbot/plugins/` - Dodaci na nivou projekta
- `~/.config/hackbot/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/hackbot/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/hackbot/hackbot.json`)
2. Konfiguracija projekta (`hackbot.json`)
3. Globalni direktorij dodataka (`~/.config/hackbot/plugins/`)
4. Direktorij dodataka projekta (`.hackbot/plugins/`)
...
Hackbot pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje Hackbot.
...
Spriječite hackbot da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u hackbot:
...
Pomoćnik `tool` kreira prilagođeni alat koji hackbot može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za hackbot zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://hackbot.ai/docs/sdk) za detalje.
