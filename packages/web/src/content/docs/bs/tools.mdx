...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. Hackbot dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u Hackbot.
...
:::note
Ovaj alat je dostupan samo kada je `HACKBOT_EXPERIMENTAL_LSP_TOOL=true` (ili `HACKBOT_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz Hackbot provajdera ili kada je varijabla `HACKBOT_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju Hackbot:
