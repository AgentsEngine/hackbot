---
title: Konfiguracija
description: Korištenje Hackbot JSON konfiguracije.
---

Možete konfigurirati Hackbot koristeći JSON konfiguracijski fajl.

---

## Format

Hackbot podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/hackbot`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/hackbot/hackbot.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`HACKBOT_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`hackbot.json` u projektu) - postavke specifične za projekat
5. **`.hackbot` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`HACKBOT_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.hackbot` i `~/.config/hackbot` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/hackbot` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu Hackbot konfiguraciju u `~/.config/hackbot/hackbot.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/hackbot/tui.json`.
...
Dodajte `hackbot.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se Hackbot pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `HACKBOT_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `HACKBOT_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.hackbot` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.hackbot`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`hackbot.ai/config.json`**](https://hackbot.ai/config.json).

TUI konfiguracija koristi [**`hackbot.ai/tui.json`**](https://hackbot.ai/tui.json).
...
Koristite `HACKBOT_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `hackbot.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `hackbot serve` i `hackbot web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš Hackbot server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `hackbot.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj Hackbot konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, Hackbot pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/hackbot/agents/` ili `.hackbot/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, Hackbot će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`hackbot run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/hackbot/commands/` ili `.hackbot/commands/`. [Saznajte više ovdje](/docs/commands).
...
Hackbot će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, Hackbot **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju Hackbot sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.hackbot/plugins/` ili `~/.config/hackbot/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite Hackbot da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
