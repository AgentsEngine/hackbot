...
Da biste otklonili probleme s Hackbot, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\hackbot\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `hackbot --log-level DEBUG`.

---

## Pohrana

Hackbot pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/hackbot/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\hackbot`
...
Hackbot Desktop pokreće lokalni Hackbot server (`opencode-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `Hackbot` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/hackbot/hackbot.jsonc` (ili `~/.config/hackbot/hackbot.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/hackbot/hackbot.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\hackbot\hackbot.jsonc`
...
Hackbot također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/hackbot/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\hackbot\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga Hackbot mogao ponovo izgraditi.

1. Potpuno zatvorite Hackbot Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/hackbot`
- **Linux**: obrišite `~/.cache/hackbot` (ili pokrenite `rm -rf ~/.cache/hackbot`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\hackbot`

3. Ponovo pokrenite Hackbot Desktop.

---

### Rješavanje problema sa vezom na serveru

Hackbot Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `hackbot.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `HACKBOT_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `HACKBOT_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, Hackbot Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa Hackbot karakteristikama.

---

### Obavještenja se ne prikazuju

Hackbot Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za Hackbot u postavkama vašeg OS-a, i
...
1. Zatvorite Hackbot Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka Hackbot Desktop aplikacije):

- `hackbot.settings.dat` (URL zadanog servera za desktop)
- `hackbot.global.dat` i `hackbot.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Da saznate kojim modelima imate pristup, pokrenite `hackbot models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\hackbot`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. Hackbot dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\hackbot`

2. Ponovo pokrenite Hackbot da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti Hackbot da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
Hackbot će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
