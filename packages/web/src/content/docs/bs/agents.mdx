...
Postoje dvije vrste agenata u Hackbot: primarni agenti i podagenti.
...
Hackbot dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
Hackbot dolazi sa tri ugrađena subagenta, **General**, **Explore** i **Scout**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

Hackbot dolazi sa dva ugrađena primarna agenta i tri ugrađena subagenta.
...
Agent samo za čitanje za istraživanje eksterne dokumentacije i zavisnosti. Koristite ga kada trebate klonirati repozitorij zavisnosti u Hackbot-ov upravljani cache, pregledati izvorni kod biblioteke ili uporediti lokalni kod sa upstream implementacijama bez mijenjanja vašeg radnog prostora.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `hackbot.json`:
...
- Globalno: `~/.config/hackbot/agents/`
- Po projektu: `.hackbot/agents/`

```markdown title="~/.config/hackbot/agents/review.md"
---
...
Ako temperatura nije navedena, Hackbot koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu Hackbot konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj Hackbot konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [Hackbot Zen](/docs/zen), koristili biste `hackbot/gpt-5.1-codex` za GPT 5.1 Codex.
...
```json title="hackbot.json" {3-6,9-12}
{
  "$schema": "https://hackbot.ai/config.json",
  "tools": {
    "write": true,
...
```json title="hackbot.json" {3-5,8-10}
{
  "$schema": "https://hackbot.ai/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Pokrenite `hackbot models` da vidite listu dostupnih modela.
:::
...
:::tip
Imate li agenta kojeg biste željeli podijeliti? [Pošalji PR](https://github.com/anomalyco/hackbot).
:::
...
```markdown title="~/.config/hackbot/agents/docs-writer.md"
---
...
```markdown title="~/.config/hackbot/agents/security-auditor.md"
---
