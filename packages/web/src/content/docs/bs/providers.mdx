---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u Hackbot.
---

import config from "../../../../config.mjs"
export const console = config.console

Hackbot koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj Hackbot konfiguraciji.
...
u `~/.local/share/hackbot/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem Hackbot
config.
...
Hackbot Zen je lista modela koje je obezbedio Hackbot tim koji su bili
testirano i potvrđeno da dobro radi sa Hackbot. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa Hackbot Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite hackbot i idite na [hackbot.ai/auth](https://hackbot.ai/auth).
...
Radi kao i svaki drugi provajder u Hackbot i potpuno je opcionalan za korištenje.

---

## Hackbot Go

Hackbot Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža Hackbot tim i koji su testirani i verificirani da dobro rade s Hackbot-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `Hackbot Go` i idite na [hackbot.ai/auth](https://hackbot.ai/zen).
...
Da biste koristili Amazon Bedrock s Hackbot:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete hackbot:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `hackbot.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u Hackbot.
:::
...
Možete konfigurirati hackbot za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi hackbot ispravno radio.
   :::
...
Također možete dodati modele kroz svoju hackbot konfiguraciju.
...
da pohrani token u hackbot auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
Hackbot koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate Hackbot
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`hackbot.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Prilagodite putem `hackbot.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s hackbot:
...
Za korištenje Google Vertex AI s Hackbot:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće hackbot.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) da automatski prijavite svaki Hackbot razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki Hackbot razgovor naveden kao zasebna sesija.
...
Možete konfigurirati hackbot za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati hackbot za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati hackbot za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za Hackbot. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/hackbot) za detalje.
:::
...
Da biste koristili Ollama Cloud s Hackbot:
...
3. Kopirajte API ključ za korištenje u Hackbot.
...
6. **Važno**: Prije upotrebe modela oblaka u Hackbot, morate lokalno povući informacije o modelu:
...
Hackbot Zen je lista testiranih i verifikovanih modela koju je obezbedio Hackbot tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>Hackbot Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **Hackbot Zen**.

```txt
   /connect
```

3. Unesite svoj Hackbot API ključ.
...
Također možete dodati dodatne modele putem vaše hackbot konfiguracije.
...
5. Također ih možete prilagoditi putem vaše hackbot konfiguracije. Evo primjera navođenja provajdera
...
Također možete prilagoditi modele kroz svoju hackbot konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
4. Kreirajte ili ažurirajte svoju `hackbot.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju Hackbot da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `hackbot auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite Hackbot konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj hackbot konfiguraciji.
