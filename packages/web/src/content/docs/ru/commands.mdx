...
Создайте `.hackbot/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию hackbot или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем hackbot [config](/docs/config):
...
- Глобальный: `~/.config/hackbot/commands/`
- Для каждого проекта: `.hackbot/commands/`
...
hackbot включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
