...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. hackbot yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
hackbot içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `HACKBOT_EXPERIMENTAL_LSP_TOOL=true` (veya `HACKBOT_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca hackbot provider kullanılırken veya `HACKBOT_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

hackbot başlatırken etkinleştirmek için:
