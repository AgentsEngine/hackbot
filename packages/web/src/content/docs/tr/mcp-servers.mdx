...
_Model Bağlam Protokolü_ veya MCP kullanarak hackbot'a harici araçlar ekleyebilirsiniz. hackbot hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [hackbot Config](https://hackbot.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/hackbot` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
hackbot, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde hackbot şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa hackbot, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `hackbot mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından hackbot, belirteçleri `~/.local/share/hackbot/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra hackbot'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi hackbot yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve hackbot'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
