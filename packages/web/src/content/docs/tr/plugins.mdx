---
title: Eklentiler
description: Hackbot'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek Hackbot'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya Hackbot'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.hackbot/plugins/` - Proje düzeyinde eklentiler
- `~/.config/hackbot/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/hackbot/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/hackbot/hackbot.json`)
2. Project config (`hackbot.json`)
3. Global eklenti dizini (`~/.config/hackbot/plugins/`)
4. Proje eklenti dizini (`.hackbot/plugins/`)
...
Hackbot bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir Hackbot SDK istemcisi.
...
Hackbot'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
Hackbot masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
Hackbot'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca Hackbot'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, Hackbot'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra Hackbot'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://hackbot.ai/docs/sdk) bakın.
