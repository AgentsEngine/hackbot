...
Ajan becerileri, hackbot'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
hackbot su konumlari tarar:

- Proje konfigurasyonu: `.hackbot/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/hackbot/skills/<name>/SKILL.md`
...
Proje yerel yollarinda hackbot, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.hackbot/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/hackbot/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.hackbot/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
hackbot, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`hackbot.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`hackbot.json` icinde):
