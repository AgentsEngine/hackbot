---
title: Sunucu
description: hackbot sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`hackbot serve` komutu, hackbot istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `hackbot.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `HACKBOT_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `hackbot` değeridir; değiştirmek isterseniz `HACKBOT_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `hackbot serve` hem de `hackbot web` için geçerlidir.
...
`hackbot` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
hackbot ile programatik etkileşim için hackbot sunucusunu kullanın.
:::

Bu mimari, hackbot'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`hackbot serve` ile bağımsız bir sunucu başlatabilirsiniz. hackbot TUI açıksa, `hackbot serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum hackbot [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
hackbot sunucusu aşağıdaki API'leri sunar.
