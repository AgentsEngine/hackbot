---
title: Sağlayıcılar
description: hackbot'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

hackbot, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı hackbot yapılandırmanızda yapılandırın.
...
`~/.local/share/hackbot/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları hackbot'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
Hackbot Zen, hackbot ekibi tarafından sağlanan modellerin bir listesidir.
hackbot ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz Hackbot Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, hackbot'u seçin ve [hackbot.ai/auth](https://hackbot.ai/auth)'ye gidin.
...
hackbot'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## Hackbot Go

Hackbot Go, hackbot ile iyi çalıştığı test edilmiş ve doğrulanmış, hackbot ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `Hackbot Go`'yu seçin ve [hackbot.ai/auth](https://hackbot.ai/zen) adresine gidin.
...
Amazon Bedrock'u hackbot ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   hackbot'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `hackbot.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi hackbot'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
hackbot'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   hackbot'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
:::note[uyumluluk notu]
hackbot, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. hackbot'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`hackbot.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
`hackbot.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi hackbot'la kullanmak için:
...
Google Vertex AI'yi hackbot ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları hackbot çalıştırılırken ayarlayın.
...
Helicone'un hackbot aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her hackbot görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her hackbot görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
hackbot'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla hackbot'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla hackbot'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini hackbot için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/hackbot)'a bakın.
:::
...
Ollama Cloud'u hackbot ile kullanmak için:
...
3. hackbot'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: hackbot'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
Hackbot Zen, hackbot ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>Hackbot Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **Hackbot Zen**'i arayın.

   ```txt
   /connect
   ```

3. hackbot API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   hackbot yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca hackbot yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
Modelleri hackbot yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı hackbot ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `hackbot.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları hackbot'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `hackbot auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için hackbot yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, hackbot yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
