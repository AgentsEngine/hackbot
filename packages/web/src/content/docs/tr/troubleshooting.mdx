...
hackbot ile ilgili bir sorunu ayiklamak icin once loglari ve diskte tuttugu yerel verileri kontrol edin.
...
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\hackbot\log` yapistirin

Log dosyalari zaman damgasiyla adlandirilir (ornegin `2025-01-09T123456.log`) ve en yeni 10 log dosyasi tutulur.

Daha ayrintili ayiklama bilgisi icin `--log-level` komut satiri secenegini kullanabilirsiniz. Ornek: `hackbot --log-level DEBUG`.

---

## Depolama

hackbot, oturum verilerini ve diger uygulama verilerini diskte su konumda saklar:

- **macOS/Linux**: `~/.local/share/hackbot/`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\hackbot` yapistirin
...
hackbot Desktop arka planda yerel bir hackbot sunucusu (`opencode-cli` sidecar) calistirir. Sorunlarin cogu bozuk bir plugin, hasarli cache veya hatali sunucu ayarindan kaynaklanir.

### Hızlı kontroller

- Uygulamayi tamamen kapatip yeniden acin
- Uygulama hata ekrani gosteriyorsa **Restart**'a tiklayip hata detaylarini kopyalayin
- Yalnizca macOS: `hackbot` menusu -> **Reload Webview** (arayuz bos/donukse yardimci olur)
...
- **macOS/Linux**: `~/.config/hackbot/hackbot.jsonc` (veya `~/.config/hackbot/hackbot.json`)
- **macOS/Linux** (eski kurulumlar): `~/.local/share/hackbot/hackbot.jsonc`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\hackbot\hackbot.jsonc` yapistirin
...
hackbot diskten yerel plugin de yukleyebilir. Bu dizinleri gecici olarak tasiyin (veya klasoru yeniden adlandirin) ve uygulamayi yeniden baslatin:

- **Global plugins**
  - **macOS/Linux**: `~/.config/hackbot/plugins/`
  - **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\hackbot\plugins` yapistirin
...
Pluginleri kapatmak ise yaramazsa (veya plugin kurulumu takili kaldiysa), hackbot'un cache'i yeniden olusturmasi icin cache'i temizleyin.

1. hackbot Desktop'u tamamen kapatin
2. Cache dizinini silin:

- **macOS**: Finder -> `Cmd+Shift+G` -> `~/.cache/hackbot`
- **Linux**: `~/.cache/hackbot` dizinini silin (veya `rm -rf ~/.cache/hackbot` calistirin)
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.cache\hackbot` yapistirin

3. hackbot Desktop'u yeniden baslatin

---

### Sunucu bağlantı sorunlarını düzeltin

hackbot Desktop ya kendi yerel sunucusunu baslatir (varsayilan) ya da sizin tanimladiginiz bir sunucu URL'sine baglanir.
...
`hackbot.json(c)` dosyanizda `server` bolumu varsa gecici olarak kaldirin ve desktop uygulamasini yeniden baslatin.

#### Ortam degiskenlerini kontrol edin

Ortamda `HACKBOT_PORT` ayarliysa desktop uygulamasi yerel sunucu icin o portu kullanmaya calisir.

- `HACKBOT_PORT` degerini kaldirin (veya bos bir port secin) ve yeniden baslatin
...
Windows'ta hackbot Desktop, Microsoft Edge **WebView2 Runtime** gerektirir. Uygulama bos pencereyle aciliyorsa veya hic baslamiyorsa WebView2'yi kurup/guncelleyip tekrar deneyin.

---

### Windows: Genel performans sorunları

Windows'ta yavaslik, dosya erisim sorunlari veya terminal problemleri yasiyorsaniz [WSL (Windows Subsystem for Linux)](/docs/windows-wsl) kullanmayi deneyin. WSL, hackbot ozellikleriyle daha sorunsuz calisan bir Linux ortami saglar.

---

### Bildirimler görünmüyor

hackbot Desktop sistem bildirimlerini yalnizca su durumlarda gosterir:

- Isletim sistemi ayarlarinizda hackbot icin bildirimler etkinse
...
1. hackbot Desktop'u kapatin
2. Su dosyalari bulun ve silin (hackbot Desktop uygulama veri dizininde yer alirlar):

- `hackbot.settings.dat` (desktop varsayilan sunucu URL'si)
- `hackbot.global.dat` ve `hackbot.workspace.*.dat` (son sunucular/projeler gibi UI durumu)
...
hackbot ile ilgili bir sorun yasiyorsaniz:
...
3. `hackbot upgrade` ile en guncel surumu kullandiginizdan emin olun
...
Erisiminiz olan modelleri gormek icin `hackbot models` calistirin.
...
2. Sorun surerse kayitli konfigurasyonu temizlemeyi deneyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.local\share\hackbot`
...
API cagrisi hatalari aliyorsaniz bunun nedeni guncel olmayan provider paketleri olabilir. hackbot, provider paketlerini (OpenAI, Anthropic, Google vb.) gerektikce dinamik yukler ve yerelde onbellekler.
...
1. Provider paket cache'ini temizleyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.cache\hackbot`

2. En guncel provider paketlerini yeniden kurmak icin hackbot'u yeniden baslatin

Bu, hackbot'un en yeni provider paket surumlerini indirmesini zorlar ve model parametresi/API degisikliklerinden kaynakli uyumsuzluklari cogu zaman cozer.
...
hackbot, Wayland kullandiginizi algilarsa `wl-clipboard` tercihi yapar. Aksi halde sirayla `xclip` ve `xsel` araclarini arar.
