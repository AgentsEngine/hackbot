...
hackbot ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak hackbot kendi `hackbot` temasını kullanır.
...
hackbot birden fazla yerleşik temayla gelir.
...
- hackbot'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
hackbot, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/hackbot/themes/*.json` veya `$XDG_CONFIG_HOME/hackbot/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.hackbot/themes/*.json`
4. **Geçerli çalışma dizini** - `./.hackbot/themes/*.json`
