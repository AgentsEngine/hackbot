---
title: الخادم
description: تفاعل مع خادم hackbot عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `hackbot serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل hackbot استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `hackbot.local` |
...
عيّن `HACKBOT_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `hackbot`، أو عيّن `HACKBOT_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `hackbot serve` و `hackbot web`.
...
عند تشغيل `hackbot` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم hackbot للتفاعل مع hackbot برمجيا.
:::

تتيح هذه البنية لـ hackbot دعم عدة عملاء وتمكّنك من التفاعل مع hackbot برمجيا.

يمكنك تشغيل `hackbot serve` لبدء خادم مستقل. إذا كانت واجهة hackbot في terminal (TUI)
قيد التشغيل، فسيبدأ `hackbot serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات Hackbot لـ [IDE](/docs/ide).
...
يعرض خادم hackbot واجهات API التالية.
