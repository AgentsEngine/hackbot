---
title: المزوّدون
description: استخدام أي موفّر LLM في Hackbot.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم Hackbot ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات Hackbot.
...
في `~/.local/share/hackbot/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات Hackbot.
...
Hackbot Zen هي قائمة نماذج يوفّرها فريق Hackbot وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع Hackbot. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع Hackbot Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر hackbot، ثم انتقل إلى [hackbot.ai/auth](https://hackbot.ai/auth).
...
يعمل مثل أي مزوّد آخر في Hackbot واستخدامه اختياري بالكامل.

---

## Hackbot Go

Hackbot Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق Hackbot، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع Hackbot.

1. شغّل الأمر `/connect` في TUI، واختر `Hackbot Go`، ثم انتقل إلى [hackbot.ai/auth](https://hackbot.ai/zen).
...
لاستخدام Amazon Bedrock مع Hackbot:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل hackbot:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `hackbot.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل Hackbot غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين hackbot لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل hackbot بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات hackbot.
...
تخزين الرمز في مساحة تخزين مصادقة hackbot.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم Hackbot نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر Hackbot
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`hackbot.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
لاستخدام اشتراك GitHub Copilot مع hackbot:
...
لاستخدام Google Vertex AI مع Hackbot:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر hackbot، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) لتسجيل كل محادثة Hackbot تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة Hackbot مدرجة كجلسة مستقلة.
...
يمكنك ضبط hackbot لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط hackbot لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط hackbot لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع Hackbot. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/hackbot) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع Hackbot:
...
3. انسخ مفتاح API لاستخدامه في Hackbot.
...
6. **مهم**: قبل استخدام نماذج السحابة في Hackbot، يجب سحب معلومات النموذج محليا:
...
Hackbot Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق Hackbot. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>Hackbot Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **Hackbot Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ Hackbot.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات hackbot.
...
5. يمكنك أيضا تخصيصها عبر إعدادات hackbot. إليك مثالا على تحديد مزوّد
...
يمكنك أيضا تخصيص النماذج عبر إعدادات hackbot. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع hackbot. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `hackbot.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ Hackbot بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `hackbot auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات hackbot وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات hackbot.
