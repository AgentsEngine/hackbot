...
تتيح مهارات الوكيل لـ Hackbot اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث Hackbot في هذه المواقع:

- إعدادات المشروع: `.hackbot/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/hackbot/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج Hackbot صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.hackbot/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/hackbot/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.hackbot/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض Hackbot المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `hackbot.json`:
...
**للوكلاء المدمجين** (في `hackbot.json`):
