...
مع Hackbot يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم Hackbot سمتنا `hackbot`.
...
يأتي Hackbot مع عدة سمات مدمجة.
...
- يريدون أن يطابق Hackbot مظهر terminal لديهم
...
يدعم Hackbot نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/hackbot/themes/*.json` أو `$XDG_CONFIG_HOME/hackbot/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.hackbot/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.hackbot/themes/*.json`
