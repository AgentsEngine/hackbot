...
أنشئ الملف `.hackbot/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات Hackbot أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ Hackbot:
...
- على مستوى النظام: `~/.config/hackbot/commands/`
- لكل مشروع: `.hackbot/commands/`
...
يتضمن hackbot عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
