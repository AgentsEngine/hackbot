---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di Hackbot.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Hackbot fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire Hackbot avvia la TUI per la directory corrente.
...
Quando usi la TUI di Hackbot, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a Hackbot. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da Hackbot. _Alias_: `/quit`, `/q`
...
Questo è separato da `hackbot.json`, che configura il comportamento di server/runtime.
...
- `leader_timeout` - Controlla per quanto tempo Hackbot attende dopo il leader key. Default: `2000`.
...
- `sound_pack` - ID del sound pack da usare. Default: `hackbot.default`.
