...
Puoi aggiungere strumenti esterni a Hackbot usando il _Model Context Protocol_, o MCP. Hackbot supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [Hackbot Config](https://hackbot.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/hackbot`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
Hackbot gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, Hackbot:
...
Se il server richiede autenticazione, Hackbot ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `hackbot mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, Hackbot memorizzera' i token in modo sicuro in `~/.local/share/hackbot/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in Hackbot insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione Hackbot come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare Hackbot al tuo account Sentry.
