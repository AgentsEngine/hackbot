...
Le skill degli agenti permettono a Hackbot di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
Hackbot cerca in queste posizioni:

- Config di progetto: `.hackbot/skills/<name>/SKILL.md`
- Config globale: `~/.config/hackbot/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, Hackbot risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.hackbot/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/hackbot/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.hackbot/skills/git-release/SKILL.md` cosi':
...
Hackbot elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `hackbot.json`:
...
**Per agenti integrati** (in `hackbot.json`):
