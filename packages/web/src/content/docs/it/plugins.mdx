---
title: Plugin
description: Scrivi plugin per estendere Hackbot.
---

I plugin ti permettono di estendere Hackbot agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di Hackbot.
...
- `.hackbot/plugins/` - plugin a livello progetto
- `~/.config/hackbot/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/hackbot/node_modules/`.
...
1. Config globale (`~/.config/hackbot/hackbot.json`)
2. Config di progetto (`hackbot.json`)
3. Directory plugin globale (`~/.config/hackbot/plugins/`)
4. Directory plugin di progetto (`.hackbot/plugins/`)
...
Hackbot esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di hackbot per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere hackbot.
...
:::note
Se usi l'app desktop di Hackbot, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a hackbot di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a hackbot:
...
L'helper `tool` crea uno strumento personalizzato che hackbot può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in hackbot insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://hackbot.ai/docs/sdk) per i dettagli.
