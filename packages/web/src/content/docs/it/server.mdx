---
title: Server
description: Interagisci con il server hackbot via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `hackbot serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client hackbot.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `hackbot.local` |
...
Imposta `HACKBOT_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `hackbot`, oppure imposta `HACKBOT_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `hackbot serve` sia per `hackbot web`.
...
Quando esegui `hackbot` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server hackbot per interagire con hackbot in modo programmatico.
:::

Questa architettura permette a hackbot di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `hackbot serve` per avviare un server standalone. Se la TUI di hackbot e gia in esecuzione, `hackbot serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di Hackbot.
...
Il server hackbot espone le seguenti API.
