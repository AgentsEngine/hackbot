...
description: Cree herramientas que LLM pueda llamar en hackbot.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de hackbot como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.hackbot/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/hackbot/tools/`.
...
```ts title=".hackbot/tools/database.ts" {1}
...
```ts title=".hackbot/tools/math.ts"
import { tool } from "@hackbot/plugin"
...
```ts title=".hackbot/tools/bash.ts"
import { tool } from "@hackbot/plugin"
...
```ts title=".hackbot/tools/project.ts" {8}
import { tool } from "@hackbot/plugin"
...
```python title=".hackbot/tools/add.py"
import sys
...
```ts title=".hackbot/tools/python-add.ts" {10}
import { tool } from "@hackbot/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".hackbot/tools/add.py")
