---
title: Complementos
description: Escriba sus propios complementos para extender Hackbot.
---

Los complementos le permiten extender Hackbot al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de Hackbot.
...
- `.hackbot/plugins/` - Complementos a nivel de proyecto
- `~/.config/hackbot/plugins/` - Complementos globales
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/hackbot/node_modules/`.
...
1. Configuración global (`~/.config/hackbot/hackbot.json`)
2. Configuración del proyecto (`hackbot.json`)
3. Directorio global de complementos (`~/.config/hackbot/plugins/`)
4. Directorio de complementos del proyecto (`.hackbot/plugins/`)
...
```json title=".hackbot/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
Hackbot ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".hackbot/plugins/my-plugin.ts"
...
```js title=".hackbot/plugins/example.js"
...
- `client`: Un cliente SDK hackbot para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar hackbot.
...
```js title=".hackbot/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "hackbot"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio Hackbot, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que hackbot lea archivos `.env`:

```javascript title=".hackbot/plugins/env-protection.js"
...
```javascript title=".hackbot/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a hackbot:

```ts title=".hackbot/plugins/custom-tools.ts"
import { type Plugin, tool } from "@hackbot/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que hackbot puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para hackbot junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://hackbot.ai/docs/sdk) para obtener más detalles.
...
```ts title=".hackbot/plugins/compaction.ts"
import type { Plugin } from "@hackbot/plugin"
...
```ts title=".hackbot/plugins/custom-compaction.ts"
import type { Plugin } from "@hackbot/plugin"
