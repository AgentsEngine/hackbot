---
title: Servidor
description: Interactuar con el servidor hackbot a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `hackbot serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente hackbot puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `hackbot.local` |
...
Configure `HACKBOT_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `hackbot`, o configure `HACKBOT_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `hackbot serve` como a `hackbot web`.
...
Cuando ejecuta `hackbot`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor hackbot para interactuar con hackbot mediante programación.
:::

Esta arquitectura permite que hackbot admita múltiples clientes y le permite interactuar con hackbot mediante programación.

Puede ejecutar `hackbot serve` para iniciar un servidor independiente. Si tienes el
hackbot TUI ejecutándose, `hackbot serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos Hackbot [IDE](/docs/ide).
...
El servidor hackbot expone las siguientes API.
