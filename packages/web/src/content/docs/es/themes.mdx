...
Con Hackbot puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, Hackbot usa nuestro propio tema `hackbot`.
...
Hackbot viene con varios temas integrados.
...
- Quiere que Hackbot coincida con la apariencia de su terminal
...
Hackbot admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/hackbot/themes/*.json` o `$XDG_CONFIG_HOME/hackbot/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.hackbot/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.hackbot/themes/*.json`
