...
Las herramientas permiten que LLM realice acciones en su código base. Hackbot viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en Hackbot.
...
:::note
Esta herramienta solo está disponible cuando `HACKBOT_EXPERIMENTAL_LSP_TOOL=true` (o `HACKBOT_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor Hackbot o cuando la variable de entorno `HACKBOT_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar Hackbot:

```bash
HACKBOT_ENABLE_EXA=1 hackbot
```
