...
Para depurar problemas con Hackbot, comience verificando los registros y los datos locales que almacena en el disco.
...
- **macOS/Linux**: `~/.local/share/hackbot/log/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\hackbot\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `hackbot --log-level DEBUG`.

---

## Almacenamiento

hackbot almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/hackbot/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\hackbot`
...
Hackbot Desktop ejecuta un servidor Hackbot local (el sidecar `opencode-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `Hackbot` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/hackbot/hackbot.jsonc` (o `~/.config/hackbot/hackbot.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/hackbot/hackbot.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\hackbot\hackbot.jsonc`
...
Hackbot también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.config/hackbot/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\hackbot\plugins`
- **Complementos de proyecto** (solo si usas la configuración por proyecto)
  - `<your-project>/.hackbot/plugins/`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que Hackbot pueda reconstruirlo.

1. Salga de Hackbot Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/hackbot`
- **Linux**: eliminar `~/.cache/hackbot` (o ejecutar `rm -rf ~/.cache/hackbot`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\hackbot`

3. Reinicie el escritorio Hackbot.

---

### Solucionar problemas de conexión del servidor

Hackbot Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `hackbot.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.

#### Verificar variables de entorno

Si tiene `HACKBOT_PORT` configurado en su entorno, la aplicación de escritorio intentará usar ese puerto para el servidor local.

- Desarme `HACKBOT_PORT` (o elija un puerto libre) y reinicie.
...
En Windows, el escritorio Hackbot requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de Hackbot.

---

### Notificaciones que no se muestran

Hackbot Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para Hackbot en la configuración de su sistema operativo, y
...
1. Salga del escritorio Hackbot.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio Hackbot):

- `hackbot.settings.dat` (URL del servidor predeterminado de escritorio)
- `hackbot.global.dat` y `hackbot.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
Si tiene problemas con Hackbot:
...
1. **Informar problemas el GitHub**
   ...
   [**github.com/anomalyco/hackbot/issues**](https://github.com/anomalyco/hackbot/issues)
...
2. **Únete a nuestro Discord**
   ...
   [**hackbot.ai/discord**](https://hackbot.ai/discord)
...
3. Asegúrese de tener la última versión con `hackbot upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `hackbot models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   ```bash
   rm -rf ~/.local/share/hackbot
   ```

   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\hackbot`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. hackbot instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\hackbot`

2. Reinicie hackbot para reinstalar los paquetes del proveedor más recientes.

Esto obligará a hackbot a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
hackbot detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
