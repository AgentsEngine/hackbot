...
Las habilidades del agente permiten a Hackbot descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
Hackbot busca estas ubicaciones:

- Configuración del proyecto: `.hackbot/skills/<name>/SKILL.md`
- Configuración global: `~/.config/hackbot/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, Hackbot sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.hackbot/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/hackbot/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.hackbot/skills/git-release/SKILL.md` así:
...
Hackbot enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `hackbot.json`:
...
**Para agentes integrados** (en `hackbot.json`):
