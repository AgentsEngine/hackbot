---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć hackbot.
---

Wtyczki umożliwiające wyłączenie hackbot poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania hackbot.
...
- `.hackbot/plugins/` - Wtyczki na poziomie projektu
- `~/.config/hackbot/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/hackbot/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/hackbot/hackbot.json`)
2. Konfiguracja projektu (`hackbot.json`)
3. Globalny katalog wtyczek (`~/.config/hackbot/plugins/`)
4. Katalog wtyczek projektu (`.hackbot/plugins/`)
...
hackbot uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z hackbot do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do hackbot.
...
:::note
Jeśli korzystasz z aplikacji komputerowej hackbot, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj hackbot czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do hackbot:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod hackbot. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://hackbot.ai/docs/sdk).
