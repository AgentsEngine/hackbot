...
Aby debugować problemy z hackbot, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\hackbot\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `hackbot --log-level DEBUG`.

---

## Przechowywanie danych

hackbot przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/hackbot/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\hackbot`
...
hackbot Desktop uruchamia w tle lokalny serwer hackbot (sidecar `opencode-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `Hackbot` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/hackbot/hackbot.jsonc` (lub `~/.config/hackbot/hackbot.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/hackbot/hackbot.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\hackbot\hackbot.jsonc`
...
hackbot może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/hackbot/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\hackbot\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby hackbot mógł ją odbudować.

1. Całkowicie zamknij hackbot Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/hackbot`
- **Linux**: usuń `~/.cache/hackbot` (lub uruchom `rm -rf ~/.cache/hackbot`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\hackbot`

3. Uruchom ponownie hackbot Desktop.

---

### Napraw problemy z połączeniem z serwerem

hackbot Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `hackbot.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `HACKBOT_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `HACKBOT_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
W systemie Windows hackbot Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami hackbot.

---

### Brak powiadomień

hackbot Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla hackbot w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie hackbot Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji hackbot Desktop):

- `hackbot.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `hackbot.global.dat` i `hackbot.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
3. Upewnij się, że masz najnowszą wersję z `hackbot upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `hackbot models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\hackbot`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. hackbot dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\hackbot`

2. Uruchom ponownie kod hackbot, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to hackbot do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
hackbot wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
