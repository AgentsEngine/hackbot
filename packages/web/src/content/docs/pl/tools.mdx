...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. Hackbot posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w Hackbot.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `HACKBOT_EXPERIMENTAL_LSP_TOOL=true` (lub `HACKBOT_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy Hackbot lub gdy zmienna środowiskowa `HACKBOT_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu Hackbot:
