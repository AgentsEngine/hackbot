---
title: Serwer
description: Interakcja z serwerem hackbot za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `hackbot serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient hackbot.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `hackbot.local` |
...
Ustaw `HACKBOT_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `hackbot` lub ustaw `HACKBOT_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `hackbot serve`, jak i `hackbot web`.
...
Kiedy uruchomisz `hackbot`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera hackbot do programowej interakcji z hackbot.
:::

Ta architektura umożliwia obsługę wielu klientów przez hackbot i programową interakcję z kodem otwartym.

Możesz uruchomić `hackbot serve`, aby uruchomić samodzielny serwer. Jeśli masz
hackbot TUI działa, `hackbot serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki hackbot [IDE](/docs/ide).
...
Serwer hackbot udostępnia następujące interfejsy API.
