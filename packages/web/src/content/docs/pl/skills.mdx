...
Umiejętności agenta pozwalają hackbot odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
hackbot przeszukuje te lokalizacje:

- Project config: `.hackbot/skills/<name>/SKILL.md`
- Global config: `~/.config/hackbot/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu hackbot przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.hackbot/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/hackbot/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.hackbot/skills/git-release/SKILL.md` w ten sposób:
...
hackbot wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `hackbot.json`:
...
**Dla agentów wbudowanych** (w `hackbot.json`):
