---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w hackbot.
---

import config from "../../../../config.mjs"
export const console = config.console

hackbot używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji hackbot.
...
w `~/.local/share/hackbot/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim hackbot
config.
...
Hackbot Zen to lista modeli dostarczonych przez zespół hackbot, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z hackbot. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od Hackbot Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `Hackbot Zen` i przejdź do [hackbot.ai/auth](https://hackbot.ai/zen).
...
Działa jak każdy inny dostawca w hackbot i jest całkowicie opcjonalny w użyciu.

---

## Hackbot Go

Hackbot Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół hackbot, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z hackbot.

1. Uruchom polecenie `/connect` w TUI, wybierz `Hackbot Go` i przejdź do [hackbot.ai/auth](https://hackbot.ai/zen).
...
Aby używać Amazon Bedrock z hackbot:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania hackbot:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `hackbot.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w hackbot nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować hackbot do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod hackbot działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji hackbot.
...
hackbot używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować hackbot
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`hackbot.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Aby korzystać z subskrypcji GitHub Copilot z kodem hackbot:
...
Aby używać Google Vertex AI z hackbot:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania hackbot.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą hackbot, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session), aby automatycznie rejestrować każdą konwersację hackbot jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja hackbot będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować hackbot tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować hackbot tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować hackbot tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem hackbot. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/hackbot).
:::
...
Aby korzystać z Ollama Cloud z hackbot:
...
3. Skopiuj klucz API do użycia w hackbot.
...
6. **Ważne**: Przed użyciem modeli chmurowych w hackbot musisz pobrać informacje o modelu lokalnie:
...
Hackbot Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół hackbot. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>Hackbot Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **Hackbot Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API hackbot.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji hackbot.
...
5. Możesz je także dostosować za pomocą konfiguracji hackbot. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z hackbot:
...
Możesz także dostosować modele za pomocą konfiguracji hackbot. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z hackbot. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `hackbot.json` w katalogu projektu:
...
Pola `limit` pozwalają hackbot zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `hackbot auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację hackbot i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji hackbot.
