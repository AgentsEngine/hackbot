...
Dzięki hackbot możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie hackbot używa naszego własnego motywu `hackbot`.
...
hackbot ma kilka wbudowanych motywów.
...
- Chcą, aby Hackbot pasował do wyglądu ich terminala
...
hackbot obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/hackbot/themes/*.json` lub `$XDG_CONFIG_HOME/hackbot/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.hackbot/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.hackbot/themes/*.json`
