...
Możesz dodać dodatkowe informacje do hackbot za pomocą _Model Context Protocol_, w skrócie MCP. hackbot obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [hackbot Config](https://hackbot.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/hackbot`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
hackbot automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, hackbot:
...
Jeśli serwer wymaga uwierzytelnienia, hackbot poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `hackbot mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji hackbot bezpieczne przechowa tokeny w `~/.local/share/hackbot/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w hackbot, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie hackbot, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć hackbot z kontem Sentry.
