---
title: Server
description: Interact with hackbot server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `hackbot serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an hackbot client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `hackbot.local` |
...
Set `HACKBOT_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `hackbot`, or set `HACKBOT_SERVER_USERNAME` to override it. This applies to both `hackbot serve` and `hackbot web`.
...
When you run `hackbot` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the hackbot server to interact with hackbot programmatically.
:::

This architecture lets hackbot support multiple clients and allows you to interact with hackbot programmatically.

You can run `hackbot serve` to start a standalone server. If you have the
hackbot TUI running, `hackbot serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Hackbot [IDE](/docs/ide) plugins.
...
The hackbot server exposes the following APIs.
