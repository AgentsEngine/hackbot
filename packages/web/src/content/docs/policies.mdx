---
title: Policies
description: Control which configured resources Hackbot may use.
---

Policies control whether Hackbot may perform an action on a named resource. This feature is experimental and is configured with the `experimental.policies` array in `hackbot.json`.

Policies are separate from [permissions](/docs/permissions). Permissions control what tools can do during a session, while policies control whether Hackbot may use a resource such as an LLM provider.
...
Hackbot currently supports one policy action:
