...
Du kan legge til eksterne verktøy i Hackbot ved å bruke _Model Context Protocol_ eller MCP. Hackbot støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [Hackbot Config](https://hackbot.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="hackbot.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/hackbot`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
Hackbot håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil Hackbot:
...
Hvis serveren krever autentisering, vil Hackbot be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `hackbot mcp auth <server-name>`.
...
```bash
hackbot mcp auth my-oauth-server
```
...
```bash
hackbot mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil Hackbot lagre tokens sikkert i `~/.local/share/hackbot/mcp-auth.json`.
...
hackbot mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i Hackbot, sammen med innebygde verktøy. Så du kan administrere dem gjennom Hackbot-konfigurasjonen som alle andre verktøy.
...
```json title="hackbot.json" {11, 14-18}
{
  "$schema": "https://hackbot.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
hackbot mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble Hackbot til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver Hackbot-samtale oppført som en separat økt.
