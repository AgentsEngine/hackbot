---
title: Tjener
description: Samhandle med Hackbot-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `hackbot serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en Hackbot-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `hackbot.local` |
...
Sett `HACKBOT_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `hackbot` som standard, eller sett `HACKBOT_SERVER_USERNAME` for å overstyre det. Dette gjelder både `hackbot serve` og `hackbot web`.
...
Når du kjører `hackbot` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk Hackbot-serveren til å samhandle med Hackbot programmatisk.
:::

Denne arkitekturen lar Hackbot støtte flere klienter og lar deg samhandle med Hackbot programmatisk.

Du kan kjøre `hackbot serve` for å starte en frittstående server. Hvis du har
Hackbot TUI kjører, vil `hackbot serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av Hackbot [IDE](/docs/ide) plugins.
