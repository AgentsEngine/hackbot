...
description: Skriv dine egne programtillegg for å utvide Hackbot.
---

Plugins lar deg utvide Hackbot ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til Hackbot.
...
- `.hackbot/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/hackbot/node_modules/`.
...
1. Global konfigurasjon (`~/.config/hackbot/hackbot.json`)
2. Prosjektkonfigurasjon (`hackbot.json`)
3. Global plugin-katalog (`~/.config/hackbot/plugins/`)
4. Prosjektpluginkatalog (`.hackbot/plugins/`)
...
Hackbot kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En Hackbot SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide Hackbot.
...
:::note
Hvis du bruker Hackbot-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre Hackbot fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til Hackbot:
...
`tool`-hjelperen lager et tilpasset verktøy som Hackbot kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for Hackbot sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://hackbot.ai/docs/sdk) for detaljer.
