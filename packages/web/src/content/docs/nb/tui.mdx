...
Hackbot gir et interaktivt terminalgrensesnitt (TUI) for å jobbe med prosjektene dine med en LLM.

Å kjøre Hackbot starter TUI for gjeldende katalog.
...
Når du bruker Hackbot TUI, kan du skrive `/` etterfulgt av et kommandonavn for raskt å utføre handlinger. For eksempel:
...
Legg til en leverandør til Hackbot. Lar deg velge fra tilgjengelige leverandører og legge til deres API-nøkler.
...
Avslutt Hackbot. _Aliaser_: `/quit`, `/q`
...
Dette er atskilt fra `hackbot.json`, som konfigurerer server-/kjøretidsoppførsel.
...
- `leader_timeout` - Styrer hvor lenge Hackbot venter etter leader key. Standard er `2000`.
...
- `sound_pack` - Sound pack-ID som skal brukes. Standard er `hackbot.default`.
