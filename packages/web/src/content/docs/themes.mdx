...
With Hackbot you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Hackbot uses our own `hackbot` theme.
...
Hackbot comes with several built-in themes.
...
- Want Hackbot to match their terminal's appearance
...
Hackbot supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/hackbot/themes/*.json` or `$XDG_CONFIG_HOME/hackbot/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.hackbot/themes/*.json`
4. **Current working directory** - Defined in `./.hackbot/themes/*.json`
