---
title: TUI
description: Using the Hackbot terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Hackbot provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running Hackbot starts the TUI for the current directory.
...
When using the Hackbot TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to Hackbot. Allows you to select from available providers and add their API keys.
...
This is separate from `hackbot.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long Hackbot waits after the leader key. Defaults to `2000`.
...
Use `HACKBOT_TUI_CONFIG` to load a custom TUI config path.
...
- `sound_pack` - Sound pack ID to use. Defaults to `hackbot.default`.
