...
Delete `packages/hackbot/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/hackbot/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/hackbot/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/hackbot`
- TUI plugin loader tests in `packages/hackbot` if runtime plugin API wiring changed
