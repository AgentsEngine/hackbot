...
- The server still lives in `packages/hackbot`.
...
Until enough shared service code lives outside `packages/hackbot`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/hackbot`

It should not import `packages/hackbot` services while `packages/hackbot`
...
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/hackbot` implementation details.
