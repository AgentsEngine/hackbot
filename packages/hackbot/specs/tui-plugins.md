...
- Author package entrypoint is `@hackbot/plugin/tui`.
...
- Import types from `@hackbot/plugin/tui`.
- `@hackbot/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@hackbot/plugin/tui"
...
When `plugin` entries exist in a writable `.opencode` dir or `HACKBOT_CONFIG_DIR`, OpenCode installs `@hackbot/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@hackbot/plugin/tui`.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@hackbot/plugin/tui`.
...
- `--pure` / `HACKBOT_PURE` skips external TUI plugins only.
