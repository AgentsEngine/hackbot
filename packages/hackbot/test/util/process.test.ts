...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.HACKBOT_TEST ?? "")'), {
      env: {
        HACKBOT_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "HACKBOT_TEST_SHELL"], {
      shell: true,
      env: {
        HACKBOT_TEST_SHELL: "ok",
      },
    })

    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("HACKBOT_TEST_SHELL=ok")
  })
