...
//   - HACKBOT_CONFIG_CONTENT      : provider config inline, no files to find
//   - HACKBOT_TEST_HOME           : pins os.homedir() → tmpdir
//   - HACKBOT_DISABLE_PROJECT_CONFIG : skip walking up for opencode.json
//   - HACKBOT_PURE                : skip external plugin discovery + install
//   - HACKBOT_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    HACKBOT_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    HACKBOT_CONFIG_CONTENT: configJson,
    HACKBOT_DISABLE_PROJECT_CONFIG: "1",
    HACKBOT_PURE: "1",
    HACKBOT_DISABLE_AUTOUPDATE: "1",
    HACKBOT_DISABLE_AUTOCOMPACT: "1",
    HACKBOT_DISABLE_MODELS_FETCH: "1",
    HACKBOT_AUTH_CONTENT: "{}",
  }
}
  ...
  return Effect.gen(function* () {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          HACKBOT_CONFIG_CONTENT: JSON.stringify({
    ...
    const serve = Effect.fn("opencode.serve")(function* (opts?: ServeOpts) {
      ...
      //   "hackbot server listening on http://<host>:<port>"
