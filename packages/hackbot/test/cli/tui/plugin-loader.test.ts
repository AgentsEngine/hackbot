...
import type { TuiAttentionSoundPack } from "hackbot-plugin/tui"
...
const { allThemes, addTheme } = await import("hackbot-tui/context/theme")
...
`,
  ...
  process.env.HACKBOT_PLUGIN_META_FILE = path.join(tmp.path, "plugin-meta.json")
  const config = createTuiResolvedConfig({
    plugin: [
...
`,
    ...
    const list = await Filesystem.readJson<Record<string, { themes?: Record<string, { dest: string }> }>>(
      process.env.HACKBOT_PLUGIN_META_FILE!,
    )
