...
describe("websearch provider", () => {
  ...
  test("supports an operational override", () => {
    const original = process.env.HACKBOT_WEBSEARCH_PROVIDER

    try {
      process.env.HACKBOT_WEBSEARCH_PROVIDER = "parallel"
      expect(selectWebSearchProvider(SESSION_ID)).toBe("parallel")

      process.env.HACKBOT_WEBSEARCH_PROVIDER = "exa"
      expect(selectWebSearchProvider(SESSION_ID)).toBe("exa")
    } finally {
      if (original === undefined) delete process.env.HACKBOT_WEBSEARCH_PROVIDER
      else process.env.HACKBOT_WEBSEARCH_PROVIDER = original
    }
  })
