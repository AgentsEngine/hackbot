...
const cleanState = Effect.gen(function* () {
  ...
  delete process.env.HACKBOT_CONFIG
  delete process.env.HACKBOT_TUI_CONFIG
...
it.instance("project config takes precedence over HACKBOT_TUI_CONFIG (matches HACKBOT_CONFIG)", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
...
it.instance("HACKBOT_TUI_CONFIG provides settings when no project config exists", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
...
it.instance("does not derive tui path from HACKBOT_CONFIG", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
