...
import { WorkspaceTable } from "hackbot-core/control-plane/workspace.sql"
...
const originalEnv = {
  HACKBOT_AUTH_CONTENT: process.env.HACKBOT_AUTH_CONTENT,
  HACKBOT_EXPERIMENTAL_WORKSPACES: process.env.HACKBOT_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.HACKBOT_EXPERIMENTAL_WORKSPACES = "true"
})
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.HACKBOT_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        ...
        expect(JSON.parse(recorded.calls.create[0].env.HACKBOT_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.HACKBOT_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.HACKBOT_EXPERIMENTAL_WORKSPACES).toBe("true")
