...
const createEmbeddedWebUIBundle = async () => {
  ...
  await $`HACKBOT_CHANNEL=${Script.channel} bun run --cwd ${appDir} build`
...
for (const item of targets) {
  ...
  await Bun.build({
    ...
    files: {
      ...
      ...(embeddedFileMap ? { "hackbot-web-ui.gen.ts": embeddedFileMap } : {}),
    },
    entrypoints: [
      "./src/index.ts",
      workerPath,
      treeSitterWorkerPath,
      ...(embeddedFileMap ? ["hackbot-web-ui.gen.ts"] : []),
    ],
    define: {
      FFF_LIBC: JSON.stringify(item.abi === "musl" ? "musl" : "gnu"),
      HACKBOT_VERSION: `'${Script.version}'`,
      HACKBOT_MODELS_DEV: generated.modelsData,
      OTUI_TREE_SITTER_WORKER_PATH: bunfsRoot + treeSitterWorkerPath,
      HACKBOT_WORKER_PATH: workerPath,
      HACKBOT_CHANNEL: `'${Script.channel}'`,
      HACKBOT_LIBC: item.os === "linux" ? `'${item.abi ?? "glibc"}'` : "",
