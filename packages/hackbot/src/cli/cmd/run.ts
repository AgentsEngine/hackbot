import type { PermissionV1 } from "hackbot-core/v1/permission"
...
import { createOpencodeClient, type OpencodeClient, type ToolPart } from "hackbot-sdk/v2"
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run hackbot with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running hackbot server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to HACKBOT_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to HACKBOT_SERVER_USERNAME or 'opencode')",
      })
      .option("dir", {
        type: "string",
