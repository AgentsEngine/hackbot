...
import { win32InstallCtrlCGuard } from "hackbot-tui/terminal-win32"

declare global {
  const HACKBOT_WORKER_PATH: string
}
...
async function target() {
  if (typeof HACKBOT_WORKER_PATH !== "undefined") return HACKBOT_WORKER_PATH
...
export const TuiThreadCommand = cmd({
  command: "$0 [project]",
  describe: "start hackbot tui",
  builder: (yargs) =>
    withNetworkOptions(yargs)
      .positional("project", {
        type: "string",
        describe: "path to start hackbot in",
      })
      .option("model", {
        type: "string",
        alias: ["m"],
