...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .hackbot/ subdirectory too)
  const candidates = [path.join(baseDir, "hackbot.json"), path.join(baseDir, "hackbot.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".hackbot", "hackbot.json"), path.join(baseDir, ".hackbot", "hackbot.jsonc"))
  }
