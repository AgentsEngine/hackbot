...
src/session/
  ...
  llm/
    ...
    ai-sdk.ts               AI SDK fullStream -> hackbot-llm LLMEvent adapter
    native-request.ts       opencode/AI SDK-shaped input -> hackbot-llm LLMRequest
...
- `HACKBOT_EXPERIMENTAL_NATIVE_LLM=true` or the umbrella `HACKBOT_EXPERIMENTAL=true` opts in. Native is not a global replacement.
