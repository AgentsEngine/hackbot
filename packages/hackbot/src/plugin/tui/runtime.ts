...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".hackbot"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".hackbot", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".hackbot", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.HACKBOT_PURE ? [] : pluginOrigins
    if (Flag.HACKBOT_PURE && pluginOrigins.length) {
    }
