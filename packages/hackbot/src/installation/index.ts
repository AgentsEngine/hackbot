...
import { InstallationChannel, InstallationVersion } from "@hackbot/core/installation/version"
import { NpmConfig } from "@hackbot/core/npm-config"
import { InstallationEvent } from "@hackbot/schema/installation-event"
...
export class Service extends Context.Service<Service, Interface>()("@hackbot/Installation") {}
...
const layer: Layer.Layer<Service, never, HttpClient.HttpClient | AppProcess.Service> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const getBrewFormula = Effect.fnUntraced(function* () {
      const tapFormula = yield* text(["brew", "list", "--formula", "hackbot/tap/opencode"])
      if (tapFormula.includes("opencode")) return "hackbot/tap/opencode"
    ...
    const result: Interface = {
      ...
      method: Effect.fn("Installation.method")(function* () {
        if (process.execPath.includes(path.join(".hackbot", "bin"))) return "curl" as Method
      ...
      latest: Effect.fn("Installation.latest")(function* (installMethod?: Method) {
        ...
        const response = yield* httpOk.execute(
          HttpClientRequest.get("https://api.github.com/repos/agentsengine/hackbot/releases/latest").pipe(
            HttpClientRequest.acceptJson,
          ),
        )
      ...
      upgrade: Effect.fn("Installation.upgrade")(function* (m: Method, target: string) {
        ...
        switch (m) {
          ...
          case "brew": {
            ...
            if (formula.includes("/")) {
              const tap = yield* run(["brew", "tap", "hackbot/tap"], { env })
              if (tap.code !== 0) {
                upgradeResult = tap
                break
              }
              const repo = yield* text(["brew", "--repo", "hackbot/tap"])
              const dir = repo.trim()
              if (dir) {
