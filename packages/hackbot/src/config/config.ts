...
import { InstallationLocal, InstallationVersion } from "@hackbot/core/installation/version"
import { existsSync } from "fs"
import { Account } from "@/account/account"
import { isRecord } from "@/util/record"
import type { ConsoleState } from "@hackbot/core/v1/config/console-state"
...
import { RemoteAuthError } from "@hackbot/core/v1/config/error"
...
export class Service extends Context.Service<Service, Interface>()("@hackbot/Config") {}
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.HACKBOT_CONFIG && !Flag.HACKBOT_CONFIG_DIR && !Flag.HACKBOT_CONFIG_CONTENT) {
      ...
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "hackbot.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "hackbot.jsonc"), env))
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "HACKBOT_CONFIG_CONTENT") return "local"
        ...
        if (Flag.HACKBOT_CONFIG) {
          yield* merge(Flag.HACKBOT_CONFIG, yield* loadFile(Flag.HACKBOT_CONFIG, authEnv))
          yield* Effect.logDebug("loaded custom config", { path: Flag.HACKBOT_CONFIG })
        }

        if (!Flag.HACKBOT_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("hackbot", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        if (Flag.HACKBOT_CONFIG_DIR) {
          yield* Effect.logDebug("loading config from HACKBOT_CONFIG_DIR", { path: Flag.HACKBOT_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".hackbot") || dir === Flag.HACKBOT_CONFIG_DIR) {
            for (const file of ["hackbot.json", "hackbot.jsonc"]) {
          ...
          // Auto-discovered plugins under `.hackbot/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.HACKBOT_CONFIG_CONTENT) {
          const source = "HACKBOT_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.HACKBOT_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          yield* Effect.logDebug("loaded custom config from HACKBOT_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["HACKBOT_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("HACKBOT_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.HACKBOT_PERMISSION) {
          try {
            result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.HACKBOT_PERMISSION))
          } catch (err) {
            yield* Effect.logWarning("HACKBOT_PERMISSION contains invalid JSON, skipping", { err })
          }
        }
        ...
        if (Flag.HACKBOT_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.HACKBOT_DISABLE_PRUNE) {
