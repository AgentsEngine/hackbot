...
import { TuiKeybind } from "@hackbot/tui/config/keybind"
import { InstallationLocal, InstallationVersion } from "@hackbot/core/installation/version"
import { makeRuntime } from "@hackbot/core/effect/runtime"
...
export class Service extends Context.Service<Service, Interface>()("@hackbot/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old hackbot.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.hackbot`
  // folders between cwd and home, and HACKBOT_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.HACKBOT_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit HACKBOT_TUI_CONFIG override, if set.
  if (Flag.HACKBOT_TUI_CONFIG) {
    const configFile = Flag.HACKBOT_TUI_CONFIG
  ...
  // 4. `.hackbot` directories (and HACKBOT_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".hackbot") || dir === Flag.HACKBOT_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".hackbot") && dir !== Flag.HACKBOT_CONFIG_DIR) continue
