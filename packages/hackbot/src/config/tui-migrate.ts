...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from hackbot.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const files = await hackbotFiles(input)
...
async function hackbotFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "hackbot"),
    ...(await Filesystem.findUp(["hackbot.json", "hackbot.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "hackbot"))
  }
  if (Flag.HACKBOT_CONFIG) files.push(Flag.HACKBOT_CONFIG)
