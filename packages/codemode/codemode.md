...
This is the living design and status document for `@hackbot/codemode` and its existing V2 OpenCode adapter.
...
`packages/hackbot/src/tool/code-mode.ts`, where nested MCP calls run the `tool.execute.before` and
