...
async function build() {
  ...
  try {
    await $`./script/build.ts --single`.cwd("packages/hackbot")
    return true
  } catch (err) {
...
async function fix(pr: PR, files: string[], prs: PR[], applied: number[], idx: number) {
  ...
  try {
    await $`hackbot run -m ${model} ${prompt}`
  } catch (err) {
    console.log(`  hackbot failed: ${err}`)
    return false
  }
  ...
  console.log("  Conflicts resolved with hackbot")
  return true
}
...
async function smoke(prs: PR[], applied: number[]) {
  ...
  console.log("\nTrying to fix final smoke check with hackbot...")
  ...
  const prompt = [
    ...
    "Run `./script/build.ts --single` in `packages/hackbot`.",
