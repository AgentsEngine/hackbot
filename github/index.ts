...
function useShareUrl() {
  return isMock() ? "https://dev.hackbot.local" : "https://hackbot.local"
}
...
async function getAccessToken() {
  ...
  let response
  if (isMock()) {
    response = await fetch("https://api.hackbot.local/exchange_github_app_token_with_pat", {
      method: "POST",
      headers: {
    ...
    response = await fetch("https://api.hackbot.local/exchange_github_app_token", {
      method: "POST",
      headers: {
